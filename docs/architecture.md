...
A fully separate app from `apps/client` — no game logic, no WebSocket dependency. Marketing/landing site plus account-facing auth pages (login/signup).
...
- **Auth**: real Supabase Auth via `@supabase/ssr` (`createBrowserClient` in `src/lib/supabase/client.ts`, `createServerClient` in `src/lib/supabase/server.ts`), plus `src/proxy.ts` (Next.js 16's replacement for `middleware.ts` — see the [proxy convention docs](https://nextjs.org/docs/app/api-reference/file-conventions/proxy)) to refresh the session on every request. Server-side code must call `supabase.auth.getUser()`, never `getSession()`, to establish who's signed in — `getUser()` revalidates the token against the Auth server instead of trusting the cookie. `apps/website` now has real Supabase usage, unlike earlier in the project.
