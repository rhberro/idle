...
- **Styling**: Chakra UI v3 — see [ADR 0004](adr/0004-chakra-ui-in-client.md) for why this app moved off Tailwind. `apps/client` has its own local Chakra system configuration, separate from `apps/website`'s (see the Website section below for why the two aren't shared).
...
This is a deliberate per-app override of the root `CLAUDE.md` frontend guidance (`Bun.serve()` + HTML imports, no `vite`):
