...
- **World/grid rendering**: PixiJS (WebGL) — tiles, sprites, animations, living as a sibling to the React tree, not a separate mount point.
- **UI chrome** (menus, inventory, automation config screens): React.
- **State**: Zustand, fed by WebSocket messages from `game-server`.
- **Styling**: Chakra UI v3 — see [ADR 0004](adr/0004-chakra-ui-in-client.md) for why this app moved off Tailwind, and [ADR 0005](adr/0005-chakra-defaults-no-tailwind-faithfulness.md) for the current state: `apps/client` runs Chakra's default theme unmodified (no custom tokens or palette), `strictTokens: true` is on, and the app is dark by intent (`<html class="dark">` in `apps/client/index.html`). `apps/client` has its own local Chakra system configuration, separate from `apps/website`'s (see the Website section below for why the two aren't shared).
...
**Why not merge `apps/client` into this Next.js app?** It came up: Next's route-based code splitting would let a merged app lazy-load the game bundle only on game routes. Considered and rejected — `apps/client` is a logged-in, WebSocket-driven, canvas-rendered app with no SSR/SEO surface, so Next's SSR machinery buys it nothing while forcing `'use client'`/`dynamic(..., { ssr: false })` discipline throughout the game code. Merging would also replace `apps/client`'s Bun-native HTML-import dev loop with Turbopack and couple the two apps' builds/deploys together — a game build break would take down the marketing site and vice versa. The one practical win (no cross-app redirect for login→play) is largely moot since both apps already validate the same Supabase session/JWT. Both apps now use Chakra UI as their design system (see [ADR 0004](adr/0004-chakra-ui-in-client.md)), but with separate, unshared theme configurations — see below — so that's no longer a reason to merge either.

This is a deliberate per-app override of the root `AGENTS.md` frontend guidance (`Bun.serve()` + HTML imports, no `vite`):
...
- **Design system**: Chakra UI v3 — no Tailwind in this app. Base UI primitives (`Toaster`, `Tooltip`, `ColorModeButton`, …) live in `src/components/ui/` and are generated/regenerated with `npx @chakra-ui/cli snippet add`, not hand-written — treat that directory as vendored code, not subject to `docs/code-style.md`. `apps/client` also uses Chakra UI now (see [ADR 0004](adr/0004-chakra-ui-in-client.md)), but the two apps don't share a theme: this app's `theme.ts` customizes fonts via Next.js's `next/font` CSS variables, which don't exist outside a Next.js build, so `apps/client` maintains its own local system configuration instead. Both apps run `strictTokens: true` (see [ADR 0005](adr/0005-chakra-defaults-no-tailwind-faithfulness.md)) — raw hex and unregistered tokens are TypeScript errors in either app.
